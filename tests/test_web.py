...
def test_default_upload_limit_is_40_mb(client):
    """Server, page copy and the client-side check must all agree."""
    import pitch_analyzer.web as web_module

    assert web_module.MAX_UPLOAD_MB == 40
    assert web_module.MAX_UPLOAD_BYTES == 40 * 1024 * 1024

    page = client.get("/", auth=AUTH).text
    assert "up to 40&nbsp;MB" in page
    assert "const MAX_BYTES = 40 * 1024 * 1024" in page
